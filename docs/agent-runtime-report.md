...
- `context_builders.py` 把上下文生成从 runtime 中进一步剥离，使新增 skill 的 context builder 有明确扩展点。
- `turn_payload.py` 把 tracking 脚本输出收敛成标准 turn payload，skill 与 runner 的契约更稳定。
- memory rewrite 被明确拆成同步初始化和异步更新两条路径，异步 worker 带完整的 job 状态跟踪与失败恢复。

### 1.3 当前最需要认清的事实

当前 `track` 在 **tracking loop** 中的行为需要分层理解：

- **目标在画面中且已绑定**：loop 直接确认绑定并周期性调度 memory rewrite，**完全不经过 Pi/VLM**，这是最高频路径。
- **目标丢失（recovery mode）**：loop 调用 `process_tracking_request_direct(...)`，此时 `execute_select_tool(...)` 的 `track` 分支在非显式 ID 场景下仍走 **VLM 驱动定位**。
- 代码里存在 `_deterministic_track_result(...)`，但尚未接入 `execute_select_tool(...)` 的 `track` 主流程。
...
| 层 | 目录 | 负责什么 | 不负责什么 |
| --- | --- | --- | --- |
| Perception | `backend/perception/` + `scripts/run_tracking_perception.py` | 抽帧、检测、写 observation、提供实时帧查询 | 技能路由、用户对话决策 |
...
`scripts/run_tracking_loop.py` 实现了三层运行状态：

#### 状态一：`tracking_bound`（目标在画面中）

当最新帧中已存在绑定的 `target_id` 时，loop **不调用任何脚本/Pi**，直接：

- 输出 `tracking_bound` 状态日志
- 按 `rewrite_interval_seconds` 周期性调度 bound memory rewrite
- 更新 `latest_confirmed_frame_path`、`latest_confirmed_bbox`、`latest_target_crop`

这是最高频、最低成本的路径。

#### 状态二：`recovery`（目标丢失）

当目标在最新帧中不可见时，loop 进入 recovery mode，调用：

- `PiAgentRunner.process_tracking_request_direct(..., recovery_mode=True, missing_target_id=..., candidate_track_id_floor_exclusive=...)`

此时生成带 `recovery_mode` 标记的 `tracking_context.json`，由 `run_tracking_track.py` 判断目标是否重新出现。

#### 状态三：`idle` / `completed`

- 无 active target 时 sleep
- perception stream completed 时优雅退出

这不是绕开架构，而是 runtime 明确支持“通用路径 + 高频零成本路径 + 降级恢复路径”的三层设计。此外，loop 可选启动 WebSocket viewer stream，向外部实时广播 tracking 状态。
...
3. 若只启用 tracking 且当前没有 active target，**直接短路到 `process_tracking_init_direct(...)`**，不走 Pi
4. 否则调 `process_session(...)` 运行 Pi
5. 按 payload 回写状态

它的意义在于：  
**把“用户说了一句话”显式变成一个 turn。**  
同时，对于最明确的初始化场景，runner 提供了零 Pi 调用的快捷路径。
...
Pi prompt 里明确规定 12 条规则，核心约束包括：

- 先读 `route_context`，tracking 时优先读 `tracking_context`
- 不要直接改状态文件，只返回 patches 和 results
- 只返回一个原始 JSON 对象，不要输出 markdown 或解释性文字
- `idle` 仅用于无 skill 匹配的场景；若 skill 匹配但需要澄清，应返回 `processed` clarification payload
- 若确定性入口脚本已返回完整 payload，直接原样返回，不要手工重写字段
...
- 成功后由 runner 同步执行 `rewrite_memory.py`
- turn 完成时，session 已经有初始化 memory

#### `track`

- 成功后由 runner 通过 `_schedule_tracking_memory_rewrite(...)` 启动**异步 detached worker**
- 主 turn 不等待 memory 更新完成

#### 异步 worker 的完整机制

runner 维护了一套 tracking rewrite job 生命周期：

- 每个 job 有唯一 `job_id`，落盘到 `tracking_rewrite_jobs/<job_id>/status.json`
- 记录 `requested_at`、`started_at`、`completed_at`、`exit_code`、`pid`
- `_tracking_rewrite_pid_alive(...)` 检查 worker 是否仍在运行
- `_recover_latest_tracking_rewrite_if_stale(...)` 在 loop 每次 bound 时检查：若 worker 异常退出但状态仍为 `queued/running`，自动标记为 `failed`
- 只有最新 job 的结果才会被写回 `skill_cache`

这不是性能优化，而是业务优先级排序 + 工程可靠性设计。

### 5.9 `process_tracking_request_direct(...)`

tracking loop 在 recovery 场景下用这个方法直接跑：

- 生成带 `recovery_mode`、`missing_target_id`、`candidate_track_id_floor_exclusive` 的 `tracking_context.json`
...
### 5.10 `process_tracking_init_direct(...)`

当 `process_chat_request` 检测到“只启用了 tracking 且没有 active target”时，直接调用：

- 生成 `tracking_context.json`
- 调 `run_tracking_init.py`
- 若失败直接抛异常（初始化失败不应静默）

这是把最明确的用户意图（初始化目标）从通用 Pi 路径中剥离出来，减少不必要的模型调用。
...
### 6.3 `context_builders.py`：上下文构建的显式扩展点

`backend/agent/context_builders.py` 定义了：

- `RouteContextBuilder`
- `TrackingContextBuilder`

它们把上下文生成逻辑从 `runner.py` 中剥离出来，使新增 skill 时只需：

1. 在 `context_views.py` 中增加 `build_<skill>_context(...)`
2. 在 `context_builders.py` 中增加对应的 dataclass builder
3. 在 `runner.py` 的 `_run_pi_turn(...)` 中注册启用条件

这比把上下文拼接逻辑直接写在 runner 里更易于扩展。

### 6.4 `turn_context.json`：本轮执行入口
...
### 6.5 `route_context.json`：面向 skill routing 的最小视图
...
### 6.6 `tracking_context.json`：tracking 的专属操作视图
...
- `chat_history`
- `frames`
- `recovery_mode`（是否在找回模式）
- `missing_target_id`（丢失的目标 ID）
- `candidate_track_id_floor_exclusive`（候选过滤下限）
...
- 历史已经裁剪
- 路径已经统一
- `frames` 优先从 `LocalPerceptionService` 读取，而不是依赖可能滞后的 `recent_frames`
...
| VLM-3 | `rewrite_memory.py`（由 runner 调度异步 worker） | 初始化或更新 tracking memory | crop + frame(s) | memory JSON |
...
- **tracking loop 日常路径（目标在画面中）**：直接绑定，**不调 VLM**
- **tracking loop recovery 路径（目标丢失）**：调用 `process_tracking_request_direct`，此时若用户未显式指定 ID，仍走 VLM
- **显式 ID 指令**：优先，不调 VLM
- `_deterministic_track_result(...)` 存在于 `select_target.py`，但尚未接入 `execute_select_tool` 的 `track` 主流程

这说明系统已经实现了“高频零 VLM 调用 + 丢失时 VLM 恢复”的分层策略，而不是所有帧都依赖 VLM。
...
### 11.2 Perception 优先从 `LocalPerceptionService` 读取

`context_views.py` 中的 `_tracking_frames(...)` 现在优先调用 `_recent_frames_from_perception(...)`：

- 直接从 `LocalPerceptionService` 读取最新 camera observations
- 若 perception service 无数据，才回退到 `raw_session["recent_frames"]`

这意味着 `tracking_context.json` 中的帧数据总是尽可能新鲜，而不是依赖 session store 的写入周期。未来 skill 的 context builder 也应遵循同一模式：优先读 perception service，再回退 persisted session。
...
2. `backend/agent/context_builders.py`
3. `backend/agent/context_views.py`
4. `backend/agent/runtime.py`
5. `backend/persistence/live_session_store.py`
6. `skills/tracking/SKILL.md`
7. `skills/tracking/scripts/run_tracking_init.py`
8. `skills/tracking/scripts/run_tracking_track.py`
9. `skills/tracking/scripts/turn_payload.py`
10. `skills/tracking/scripts/select_target.py`
11. `skills/tracking/scripts/rewrite_memory.py`
