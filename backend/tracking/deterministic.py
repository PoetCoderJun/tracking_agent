...
from backend.tracking.context import build_tracking_context, tracking_state_snapshot
...
def apply_processed_tracking_payload(
    *,
    ...
    _apply_tracking_lifecycle_patch_after_payload(
        sessions=sessions,
        session_id=session_id,
        session_result=session_result,
    )
...
def _apply_tracking_lifecycle_patch_after_payload(
    *,
    sessions: AgentSessionStore,
    session_id: str,
    session_result: Dict[str, Any],
    ...
    if behavior == "init":
        if found:
            ...
            patch["generation"] = int(tracking_state.get("generation", 0) or 0) + 1
            patch["next_tracking_turn_at"] = now + DEFAULT_TRACKING_INTERVAL_SECONDS
            patch["last_completed_frame_id"] = ""
            patch["last_seen_frame_id"] = frame_id
            patch["last_trigger"] = "init"
            patch["stop_reason"] = None
            patch["lifecycle_status"] = TRACKING_LIFECYCLE_SCHEDULED
        else:
            patch["lifecycle_status"] = TRACKING_LIFECYCLE_SEEKING
    elif behavior == "track":
        patch["last_completed_frame_id"] = frame_id
        patch["next_tracking_turn_at"] = now + DEFAULT_TRACKING_INTERVAL_SECONDS
        patch["stop_reason"] = None
        patch["lifecycle_status"] = TRACKING_LIFECYCLE_BOUND if found else TRACKING_LIFECYCLE_SEEKING
...
def run_tracking_mini_turn(
    *,
    ...
    rewrite_memory_input = _as_optional_dict(payload.get("rewrite_memory_input"), "rewrite_memory_input")
    ...
    payload["rewrite_output"] = rewrite_output
...
def process_tracking_init_direct(
    *,
    ...
    try:
        ...
        try:
            payload = run_tracking_mini_turn(
                session=session,
                request_id=request_id,
                behavior="init",
                text=text,
