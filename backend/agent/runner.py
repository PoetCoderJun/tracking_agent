...
from backend.agent.pi_protocol import (
    _build_pi_prompt,
    _payload_from_messages,
    _pi_command,
    _pi_subprocess_env,
    _run_pi_turn as _execute_pi_command,
)
from backend.agent.route_context import build_route_context
...
def _schedule_rewrite_followup(
    *,
    ...
    from skills.tracking.runtime import schedule_tracking_memory_rewrite
...
class PiAgentRunner:
    ...
    def process_tracking_request_direct(
        ...
        *,
        ...
        from skills.tracking.runtime import process_tracking_request_direct

        return process_tracking_request_direct(
        ...
        *,
        ...
        for _ in range(3):
            try:
                ...
                message = str(exc)
                retryable = "valid turn payload" in message or "timed out" in message.lower()
                if not retryable:
                    raise
                last_error = exc
